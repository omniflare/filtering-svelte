<script lang="ts">
	...
	import ProductSkeleton from '$lib/components/ui/skeleton/ProductSkeleton.svelte';
	...
	function applyArrayFilter({
		...
		if (isFilterApplied) {
			filter[category] = filter[category].filter((v : any) => v !== value) as typeof filter.color &
				typeof filter.size;
		} else {
